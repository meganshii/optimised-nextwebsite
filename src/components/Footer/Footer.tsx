...
const Footer: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* <PolicyHeader /> */}
      <footer className={`${isDarkBackground ? 'bg-[#222222]' : 'bg-white'} ${styles.footer}`}>
  ...
  {isDarkBackground && <div className={`${styles.overlay} `}></div>}
          ...
          <div className="flex flex-col-reverse  lg:flex-row  justify-evenly items-start w-[100%] lg:px-5  mb-[-0.10rem] ">
            <div className="flex flex-row-reverse lg:flex-row mt-7 lg:mt-0 ">
              ...
              >
                ...
                <div className={`flex flex-col hover:text-red-600 -space-y-6 font-poppins  w-[3rem] font-light  text-xs lg:text-xs md:text-lg  `}>
                  {footerData.stores.map((store, index) => (
                    <Link key={index} href="/" passHref>
                      <p className={`${styles.footeratags} ${textColor}`}>{store}</p>
                    </Link>
                  ))}
                </div>
                <h3
                ...
                <p className={`mt-3 font-light text-xs lg:text-xs md:text-lg lg:w-[9rem] md:w-[12rem] w-[10rem] font-poppins hover:text-red-600 cursor-pointer ${textColor}`}>
                ...
                <p className={`mt-3 font-light text-xs lg:text-xs md:text-lg font-poppins hover:text-red-600 cursor-pointer  lg:w-[7rem] md:w-[10rem] w-[7rem]  ${textColor}`}>
                  India | English
                </p>
                <h3
              ...
              <div className="flex flex-col w-[18%] relative left-[10rem] lg:left-16 md:left-[18rem] ">
                <h3 className={`font-semibold font-poppins text-sm md:text-xl lg:text-sm  ${textColor}`}>
                ...
                <h3 className={`font-semibold text-sm lg:text-xs md:text-lg  font-poppins mt-5  ${textColor}`}>
                ...
                <div className="text-xs lg:text-xs md:text-lg font-light font-poppins -space-y-7 hover:text-red-600 cursor-pointer  w-[3rem]">
                  {footerData.careers.map((career, index) => (
                    <Link key={index} href="/" passHref>
                      <p className={`${styles.footeratags}   ${textColor}`}>{career}</p>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <div className="flex-row flex lg:flex-none  lg:mt-0 mt-6 lg:-ml-[30vw]">
              <div className="w-[20%] ">
                <h3 className={`font-semibold text-sm md:text-xl lg:text-sm font-poppins  ${textColor}`}>
               {footerData.linkTitle}
                ...
                <div className="font-poppins text-xs lg:text-xs md:text-lg font-light space-y-10 hover:text-red-600   md:w-[5rem]  w-[4rem]">
                  {footerData.links.map((link, index) => (
                    <Link key={index} href="/" passHref>
                      <p className={`${styles.footeratags}  ${textColor}`}>{link}</p>
                    </Link>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col  w-[30%] lg:ml-20 md:ml-[8rem] ml-10 ">
                ...
                <div className="font-poppins hover:text-red-600 font-light text-xs lg:text-xs md:text-lg w-[8.5rem] lg:w-[8.5rem] md:w-[13rem]  cursor-pointer">
                  {footerData.services.map((service, index) => (
                    <Link key={index} href="/" passHref>
                      <p className={`${styles.footeratags}  ${textColor}`}>{service}</p>
                    </Link>
                  ))}
                </div>
              </div>
              ...
              <div className="flex lg:flex-col ">
                <div className="  ">
                  ...
                  >
                    {footerData.contacts.map((contact, index) => (
                      ...
                      <p className={`${styles.footeratags}  ${textColor}`}>{contact.name}</p>
                      </Link>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="font-poppins hover:text-red-600 cursor-pointer text-xs lg:text-xs md:text-lg font-light mt-2 lg:ml-0  md:ml-[20rem]">
                    <p className={` lg:w-[14rem] w-[12rem] md:w-[15rem] hover:text-red-600 ${textColor}`}>
                      {footerData.address}
                    </p>
                  </div>
                </div>
      ...
      <div className={`${styles.footerrights} ${isDarkBackground ? 'bg-[#222222]' : 'bg-white'} relative`}>
  {isDarkBackground && <div className="absolute top-0 left-0 w-full h-full bg-black opacity-60 z-10"></div>}
